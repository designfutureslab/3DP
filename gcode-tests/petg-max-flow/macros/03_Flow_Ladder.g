...
echo "Ladder:", global.petgLadderStart, "->", global.petgLadderMax, "mm/min, +", global.petgLadderStep, "per rung,", global.petgStepSeconds, "s each."
M291 P"Run the flow ladder? Have a tray ready to catch the extrudate." R"Stage 3: Flow Ladder" S4 K{"Start","Cancel"} T0
if input != 0
...
if var.stalled
    ...
    M291 P{"Max reliable = " ^ global.petgMaxGoodFeed ^ " mm/min E (stall at " ^ var.feed ^ " mm/min). Run Stage 4 to cool down."} R"RESULT" S1 T0
else
